...
export default function UniTab({ openAssignmentDemo, onDemoClosed, assignmentsTabOverride }: UniTabProps) {
  ...
  const [editingAssignment, setEditingAssignment] = useState<{ title: string; dueDate: string; weight: string; grade: string; maxGrade: string; status: string; courseId: string; description: string } | null>(null);
  ...
  const displayCourses = useMemo(() => {
    if (openAssignmentDemo && courses.length === 0) {
      return [{ id: 999, name: "Intro to AI - Autumn 2026", code: "10101", term: "Autumn", year: 2026 }];
    }
    return courses;
  ...
  const displayAssignmentsForCharts = useMemo<AssignmentForCharts[]>(() => {
    ...
    return filtered.map(a => {
      ...
      return {
        ...
        maxGrade: a.maxGrade,
        weight: a.weight,
        courseCode: getCourseCode(a.course),
  ...
  const handleSelectedAssignmentStatusChange = async (status: string) => {
    ...
    if (editingAssignment) setEditingAssignment(prev => prev ? { ...prev, status } : prev);
    await updateAssignmentStatus(assignmentId, status);
  };

  const startEditingAssignment = (a: Assignment) => {
    const d = new Date(a.dueDate);
    const isValidDate = !isNaN(d.getTime()) && d.getFullYear() > 1970;
    setEditingAssignment({
      title: a.title,
      dueDate: isValidDate ? d.toISOString().split('T')[0] : '',
      weight: String(Math.round(a.weight * 100)),
      grade: a.grade != null ? String(a.grade) : '',
      maxGrade: String(a.maxGrade),
      status: a.status,
  ...
  const saveEditingAssignment = async () => {
    ...
    try {
      const body: Record<string, unknown> = {
        ...
        weight: Number(editingAssignment.weight) / 100,
        maxGrade: Number(editingAssignment.maxGrade),
        status: editingAssignment.status,
        description: editingAssignment.description || '',
      };
      if (editingAssignment.dueDate) body.dueDate = new Date(editingAssignment.dueDate).toISOString();
      if (editingAssignment.grade !== '') body.grade = Number(editingAssignment.grade);
      else body.grade = null;
      if (editingAssignment.courseId) body.courseId = Number(editingAssignment.courseId);
      ...
      if (res.ok) {
        ...
        setAssignments(prev => prev.map(a => a.id === selectedAssignment.id ? {
          ...a,
          title: editingAssignment.title,
          dueDate: editingAssignment.dueDate ? new Date(editingAssignment.dueDate).toISOString() : a.dueDate,
          weight: Number(editingAssignment.weight) / 100,
          maxGrade: Number(editingAssignment.maxGrade),
          grade: editingAssignment.grade !== '' ? Number(editingAssignment.grade) : null,
          status: editingAssignment.status,
          description: editingAssignment.description,
          course: { ...a.course, id: Number(editingAssignment.courseId) || a.course.id },
        } : a));
        setSelectedAssignment(prev => prev ? {
          ...prev,
          title: editingAssignment.title,
          dueDate: editingAssignment.dueDate ? new Date(editingAssignment.dueDate).toISOString() : prev.dueDate,
          weight: Number(editingAssignment.weight) / 100,
          maxGrade: Number(editingAssignment.maxGrade),
          grade: editingAssignment.grade !== '' ? Number(editingAssignment.grade) : null,
          status: editingAssignment.status,
          description: editingAssignment.description,
          course: { ...prev.course, id: Number(editingAssignment.courseId) || prev.course.id },
        } : prev);
        setEditingAssignment(null);
      }
    } catch (err) { console.error('Save assignment failed', err); }
  ...
  const notesByCourse = useMemo(() => {
    ...
    for (const note of notes) {
      ...
      const key = course ? `${getCourseDisplayName(course)} (${getCourseCode(course)})` : "General Notes";
                        ...
                        >
                          <div className="flex items-center gap-3 pl-1 min-w-0 flex-1">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="text-xs text-gray-500 dark:text-gray-400">{getCourseCode(c)}</div>
                            </div>
                          </div>
                          <button
                            type="button"
                                        ...
                                        <div className="min-w-0 flex-1 flex items-center gap-3">
                                          ...
                                          <div className="min-w-0 flex-1">
                                            <div className="flex items-center gap-2 mb-0.5">
                                              <div className="text-[9px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">{(() => { const c = displayCourses.find(c => c.id === a.course.id); return c ? getCourseDisplayName(c) : getCourseCode(a.course); })()}</div>
                                            </div>
          ...
          >
            ...
            <div className="p-8 pb-4">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {!editingAssignment && (
                    <button onClick={() => startEditingAssignment(selectedAssignment)} className="p-2 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-full transition-colors" title="Edit assignment">
                  ...
                  <button onClick={() => { setSelectedAssignment(null); setEditingAssignment(null); }} className="p-2 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-full transition-colors">
              ...
              {editingAssignment ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-gray-400 dark:text-zinc-500 tracking-wider uppercase">Due Date</label>
                    <input type="date" className="w-full rounded-xl bg-white dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-800 px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-indigo-500 text-gray-900 dark:text-white" value={editingAssignment.dueDate} onChange={e => setEditingAssignment({ ...editingAssignment, dueDate: e.target.value })} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-gray-400 dark:text-zinc-500 tracking-wider uppercase">Weight %</label>
                    <input type="number" min="0" max="100" className="w-full rounded-xl bg-white dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-800 px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-indigo-500 text-gray-900 dark:text-white" value={editingAssignment.weight} onChange={e => setEditingAssignment({ ...editingAssignment, weight: e.target.value })} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-gray-400 dark:text-zinc-500 tracking-wider uppercase">Grade</label>
                    <div className="flex items-center gap-1">
                      <input type="number" min="0" className="w-full rounded-xl bg-white dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-800 px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-indigo-500 text-gray-900 dark:text-white" placeholder="--" value={editingAssignment.grade} onChange={e => setEditingAssignment({ ...editingAssignment, grade: e.target.value })} />
                      <span className="text-gray-400 dark:text-zinc-500 text-sm">/</span>
                      <input type="number" min="1" className="w-full rounded-xl bg-white dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-800 px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-indigo-500 text-gray-900 dark:text-white" value={editingAssignment.maxGrade} onChange={e => setEditingAssignment({ ...editingAssignment, maxGrade: e.target.value })} />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-gray-400 dark:text-zinc-500 tracking-wider uppercase">Status</label>
                    <select className="w-full rounded-xl bg-white dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-800 px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-indigo-500 text-gray-900 dark:text-white" value={editingAssignment.status} onChange={e => setEditingAssignment({ ...editingAssignment, status: e.target.value })}>
                      {ASSIGNMENT_STATUS_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1 col-span-2 sm:col-span-2">
                    <label className="text-[9px] font-black text-gray-400 dark:text-zinc-500 tracking-wider uppercase">Course</label>
                    <select className="w-full rounded-xl bg-white dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-800 px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-indigo-500 text-gray-900 dark:text-white" value={editingAssignment.courseId} onChange={e => setEditingAssignment({ ...editingAssignment, courseId: e.target.value })}>
                      {displayCourses.map(c => <option key={c.id} value={c.id}>{getCourseOptionLabel(c)}</option>)}
                    </select>
                  </div>
                </div>
              ) : (
            ...
            <div className="flex-1 overflow-y-auto p-8 pt-0 space-y-6 scrollbar-hide">
              <div className="space-y-2">
                ...
                {editingAssignment ? (
                  <textarea
                    className="w-full p-4 rounded-2xl bg-white dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-800 text-sm text-gray-700 dark:text-zinc-300 leading-relaxed min-h-[120px] outline-none focus:ring-1 focus:ring-indigo-500 resize-y"
            ...
            {/* Footer Actions */}
            <div className="p-6 pt-4 border-t border-gray-100 dark:border-zinc-800 bg-gray-50/50 dark:bg-zinc-900/20 flex justify-between gap-3">
              <button
                ...
                {editingAssignment ? (
                  <>
                    ...
                    <button
                      ...
                      onClick={() => void saveEditingAssignment()}
                      className="px-6 py-2 rounded-xl text-xs font-bold bg-indigo-500 text-white hover:bg-indigo-600 transition-colors disabled:opacity-50"
                    >
                  ...
                  <button 
                    onClick={() => { setSelectedAssignment(null); setEditingAssignment(null); }}
                    className="px-6 py-2 rounded-xl text-xs font-bold text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors"
                  >
                    Close
                  </button>
            ...
            <form onSubmit={updateCourse} className="p-6 pt-4 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  inputMode="numeric"
                  maxLength={5}
                  value={getCourseCode(editingCourse)}
                  onChange={(e) => {
