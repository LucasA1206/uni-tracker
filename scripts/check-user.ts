...
async function main() {
    ...
    // Try to find the user by either username or alternatively search
    ...
    if (!user) {
        console.error(`❌ User '${username}' / '${altUsername}' not found in the database!`);

        // Look for all users
        const allUsers = await prisma.user.findMany({ select: { id: true, username: true } });
        console.log(`Found ${allUsers.length} total users in DB:`, allUsers);
        return;
    }

    console.log(`✅ User '${user.username}' found! (ID: ${user.id}, Email: ${user.universityEmail}, Name: ${user.name})`);
